...
interface PayrollContextType {
  ...
  loading: boolean;
  getCompensationPlanForUstaadh: (ustaadhId: string) => Promise<CompensationPlan | null>;
  upsertCompensationPlan: (data: any) => Promise<void>;
  recordSalaryPayment: (ustaadhId: string, month: string, amount: number) => Promise<void>;
  addSalaryAdjustment: (ustaadhId: string, adjustmentData: any) => Promise<void>;
...
export const PayrollProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const upsertCompensationPlan = useCallback(async (data: any) => {
    try {
