...
export const MessagesPage: React.FC = () => {
  const { user } = useAuth();
  const { messages, conversations, sendMessage, getConversation } = useMessaging();
              ...
              {filteredConversations.map((conversation) => (
                <button
                  ...
                  className={`w-full p-4 text-left hover:bg-gray-50 transition-colors ${
                    selectedConversation === conversation.partnerId ? 'bg-green-50 border-r-2 border-green-500' : ''
                  }`}
                >
                  <div className="flex items-center space-x-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between mb-1">
                        <p className="text-sm font-medium text-gray-900 truncate">{conversation.partnerName}</p>
                        <p className="text-xs text-gray-500">
                          {conversation.lastMessage?.timestamp ? new Date(conversation.lastMessage.timestamp).toLocaleDateString() : ''}
                        </p>
                      </div>
                      <p className="text-sm text-gray-600 truncate">{conversation.lastMessage?.content || ''}</p>
                      <p className="text-xs text-gray-500 capitalize mt-1">{conversation.partnerRole}</p>
                    </div>
                  </div>
                </button>
              ))}
          ...
          <>
            ...
            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-800">{selectedConv.partnerName}</h3>
                  <p className="text-sm text-gray-500 capitalize">{selectedConv.partnerRole}</p>
                </div>
              </div>
            </div>
